    ...
    return (
        <div className="max-w-sm mb-5 mr-5 w-80 h-52 bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
            ...
            <div className="grid grid-col-2 w-80 justify-center">
                <div className="mb-2">
                    <Link to={"/job/"+job.jobId}  state={{self: job.self, id: job.jobId}} style={{marginRight: "15px"}} className="text-sm focus:outline-none text-violet-900 bg-purple-900 bg-opacity-30 hover:bg-purple-900 hover:bg-opacity-50 font-small rounded-lg text-sm px-5 py-2.5">
                        {t("JobCard.publication")}
                    </Link>
                    {localStorage.getItem("hogar-role") === "EMPLOYER" &&
                        <Link to={"/job/applicants/"+job.jobId} state={{applicants: job.applicants, title : job.title}} className="text-sm focus:outline-none text-violet-900 bg-purple-900 bg-opacity-30 hover:bg-purple-900 hover:bg-opacity-50 font-small rounded-lg text-sm px-5 py-2.5">
